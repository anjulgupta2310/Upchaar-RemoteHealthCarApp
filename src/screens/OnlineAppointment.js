...
export default class App extends Component {
  ...
  componentDidMount() {
    var query = firebase
      .database()
      .ref('Appointment/OnlineAppointment/201851014')
      .orderByKey();
  ...
  render() {
    ...
    for (let i = 0; i < nameTemp.length; i += 1) {
      var dataRow = [];
      if(parseInt(temp[i].substring(0,2))>=state.today.getDate() && parseInt(temp[i].substring(3,5))>=state.today.getMonth()+1 && parseInt(temp[i].substring(6,10))>=state.today.getFullYear())
      {
        dataRow.push(`${nameTemp[i]._name.name}`);
        dataRow.push(`${nameTemp[i]._name.time}`);
        dataRow.push(`${nameTemp[i]._name.date}`);
        dataRow.push(`${nameTemp[i]._name.meet}`);
        data.push(dataRow);
      }
